...
function Card({ titulo, descricao, children }: { titulo: string; descricao?: string; children: React.ReactNode }) {
  return <section className="rounded-2xl bg-white p-6 shadow-[0_8px_24px_-8px_rgba(2,87,160,0.14)]"><h2 className="text-base font-black text-azul-principal">{titulo}</h2>{descricao && <p className="mt-1 text-sm text-slate-500">{descricao}</p>}<div className="mt-5">{children}</div></section>;
}

function Campo({ titulo, dica, erro, children }: { titulo: string; dica?: string; erro?: string; children: React.ReactNode }) {
  return <label className="block text-sm font-bold text-texto-principal"><span>{titulo}</span>{children}{erro ? <span className="mt-1 block text-xs font-bold text-red-600">{erro}</span> : dica && <span className="mt-1 block text-xs font-medium text-slate-400">{dica}</span>}</label>;
}

const input = "mt-2 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco";
...
function Botao({ children, variante = "principal", ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variante?: "principal" | "borda" | "perigo" }) {
  const estilo = variante === "principal" ? "bg-azul-principal text-white hover:bg-[#014884]" : variante === "perigo" ? "bg-red-600 text-white hover:bg-red-700" : "border border-azul-acao bg-white text-azul-principal hover:bg-azul-superficie";
...
export default function PainelAdministrativo() {
  ...
  if (desktop === null) return <main className="min-h-dvh bg-fundo-claro" />;
  if (!desktop) return <main className="grid min-h-dvh place-items-center bg-fundo-claro p-6 text-center"><div className="max-w-sm rounded-3xl bg-white p-8 shadow-[0_8px_24px_-8px_rgba(2,87,160,0.14)]"><School className="mx-auto size-10 text-azul-acao" /><h1 className="mt-5 text-xl font-black text-azul-principal">Área administrativa</h1><p className="mt-3 text-sm leading-6 text-slate-600">A página de administrador está disponível somente em computadores.</p></div></main>;
  ...
  const removerBeneficioGlobal = async () => {
    ...
    try {
      ...
      setModalExcluirBeneficio(null); mostrarMensagem("Auxílio removido de todas as escolas.");
    } catch (erro) {
      mostrarMensagem(erro instanceof Error && erro.message ? `Não foi possível remover o auxílio padrão: ${erro.message}` : "Não foi possível remover o auxílio padrão.", "erro");
    } finally { setProcessando(false); }
  };
  const adicionarBeneficio = async (nome: string, descricao: string) => {
    ...
    try {
      ...
      mostrarMensagem("Não foi possível criar o auxílio padrão.", "erro");
      return false;
    } finally { setProcessando(false); }
  };
  ...
  return <main className="min-h-dvh bg-fundo-claro text-texto-principal">
    {tela === "inicio" && <Inicio onAbrir={setTela} />}
    {tela === "estatisticas" && <EstatisticasRelatorios onVoltar={() => setTela("inicio")} />}
    {tela === "escolas" && <section className="mx-auto max-w-7xl px-8 py-10"><button type="button" onClick={() => setTela("inicio")} className="inline-flex items-center gap-2 text-sm font-black text-azul-acao hover:text-azul-principal"><ArrowLeft className="size-4" />Voltar ao painel</button><h1 className="mt-6 text-3xl font-black text-azul-principal">Gerenciar Escolas</h1><p className="mt-1 text-sm text-slate-500">Cadastre, edite e controle a visibilidade das unidades no mapa.</p>
...
function Inicio({ onAbrir }: { onAbrir: (tela: "escolas" | "estatisticas") => void }) {
  ...
  return <section className="mx-auto max-w-5xl px-8 py-20"><p className="text-sm font-black tracking-[0.22em] text-azul-acao">VEM PRA EJA</p><h1 className="mt-3 text-4xl font-black text-azul-principal">Painel Administrativo</h1><p className="mt-2 text-slate-500">Selecione uma área para continuar.</p><div className="mt-10 grid grid-cols-2 gap-6">{cards.map((card) => { const Icone = card.icone; return <button key={card.titulo} type="button" onClick={() => onAbrir(card.tela)} className="group rounded-3xl bg-white p-8 text-left shadow-[0_8px_24px_-8px_rgba(2,87,160,0.14)] transition hover:-translate-y-1 hover:shadow-[0_18px_35px_-12px_rgba(2,87,160,0.25)]"><span className="grid size-14 place-items-center rounded-2xl bg-azul-superficie text-azul-acao"><Icone className="size-7" /></span><h2 className="mt-6 text-xl font-black text-texto-principal">{card.titulo}</h2><p className="mt-2 min-h-12 text-sm leading-6 text-slate-500">{card.descricao}</p><span className="mt-6 inline-flex items-center gap-1 text-sm font-black text-azul-acao">Acessar <ChevronRight className="size-4 transition group-hover:translate-x-1" /></span></button>; })}</div></section>;
}

// Versão anterior mantida temporariamente como referência durante a homologação.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function Estatisticas({ onVoltar }: { onVoltar: () => void }) {
  return <section className="mx-auto max-w-5xl px-8 py-12"><button type="button" onClick={onVoltar} className="inline-flex items-center gap-2 text-sm font-black text-azul-acao"><ArrowLeft className="size-4" />Voltar ao painel</button><Card titulo="Estatísticas de Acesso" descricao="Esta área receberá os dados do Google Analytics quando ele for configurado no site."><div className="grid place-items-center py-20 text-center"><BarChart3 className="size-10 text-azul-acao" /><p className="mt-4 text-lg font-black text-azul-principal">Módulo preparado</p><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">Ainda não existem dados de acesso conectados. Quando o Google Analytics for adicionado, esta tela poderá mostrar visitas, páginas mais acessadas e origem dos acessos.</p></div></Card></section>;
}

function Aba({ ativa, children, onClick }: { ativa: boolean; children: React.ReactNode; onClick: () => void }) { return <button type="button" onClick={onClick} className={`-mb-px border-b-2 px-5 py-3 text-sm font-black ${ativa ? "border-azul-principal text-azul-principal" : "border-transparent text-slate-400 hover:text-azul-secundario"}`}>{children}</button>; }
...
function CartaoTurno({ turno, indice, niveis, anterior, catalogo, onAdicionarBeneficio, onAtualizar, onRemover, onSolicitarExcluirBeneficio }: { turno: TurnoFormulario; indice: number; niveis: Nivel[]; anterior?: TurnoFormulario; catalogo: Beneficio[]; onAdicionarBeneficio: (nome: string) => Promise<void>; onAtualizar: (turno: TurnoFormulario) => void; onRemover: () => void; onSolicitarExcluirBeneficio: (beneficio: Beneficio) => void }) {
  ...
  return <article className="rounded-2xl border border-slate-200 p-5"><div className="flex items-center justify-between"><h3 className="font-black text-azul-principal">Turno {indice + 1}</h3><div className="flex items-center gap-3">{anterior && <button type="button" onClick={() => onAtualizar({ ...turno, comoFunciona: anterior.comoFunciona, beneficios: anterior.beneficios.map((beneficio) => ({ ...beneficio })) })} className="inline-flex items-center gap-1 rounded-lg bg-violet-50 px-3 py-2 text-xs font-black text-violet-700 hover:bg-violet-100"><Copy className="size-3.5" />Copiar informações do turno anterior</button>}<button type="button" onClick={onRemover} className="rounded-lg p-2 text-red-500 hover:bg-red-50" aria-label="Remover turno"><Trash2 className="size-5" /></button></div></div><div className="mt-5 grid grid-cols-3 gap-4"><Campo titulo="Nível deste turno"><select value={turno.nivel} onChange={(evento) => onAtualizar({ ...turno, nivel: evento.target.value as Nivel })} className={input}>{niveis.map((nivel) => <option key={nivel}>{nivel}</option>)}</select></Campo><Campo titulo="Turno"><select value={turno.turno} onChange={(evento) => onAtualizar({ ...turno, turno: evento.target.value as TurnoFormulario["turno"] })} className={input}><option>Manhã</option><option>Tarde</option><option>Noite</option></select></Campo><Campo titulo="Horário" dica={`Grava como: ${turno.inicio || "HH:MM"} - ${turno.fim || "HH:MM"}`}><div className="mt-2 flex items-center gap-2"><Clock3 className="size-4 shrink-0 text-azul-secundario" /><input type="time" value={turno.inicio} onChange={(evento) => onAtualizar({ ...turno, inicio: evento.target.value })} className="w-full rounded-xl border border-slate-300 px-2 py-2.5 font-normal outline-none focus:border-azul-acao" /><span>–</span><input type="time" value={turno.fim} onChange={(evento) => onAtualizar({ ...turno, fim: evento.target.value })} className="w-full rounded-xl border border-slate-300 px-2 py-2.5 font-normal outline-none focus:border-azul-acao" /></div></Campo></div><div className="mt-5 grid grid-cols-2 gap-4"><Campo titulo="Dias de aula" dica={`Grava como: ${abreviarDias(turno.diaInicio, turno.diaFim) || "Seg - Sex"}`}><div className="mt-2 flex items-center gap-2"><span className="text-sm font-normal text-slate-500">De</span><select value={turno.diaInicio} onChange={(evento) => onAtualizar({ ...turno, diaInicio: evento.target.value as Dia })} className="w-full rounded-xl border border-slate-300 bg-white px-2 py-2.5 font-normal outline-none focus:border-azul-acao"><option value="">–</option>{DIAS.map((dia) => <option key={dia}>{dia}</option>)}</select><span className="text-sm font-normal text-slate-500">a</span><select value={turno.diaFim} onChange={(evento) => onAtualizar({ ...turno, diaFim: evento.target.value as Dia })} className="w-full rounded-xl border border-slate-300 bg-white px-2 py-2.5 font-normal outline-none focus:border-azul-acao"><option value="">–</option>{DIAS.map((dia) => <option key={dia}>{dia}</option>)}</select></div></Campo><Campo titulo="Como funciona"><textarea rows={4} value={turno.comoFunciona} onChange={(evento) => onAtualizar({ ...turno, comoFunciona: evento.target.value })} className={input} placeholder="Descreva como funciona este turno" /></Campo></div><div className="mt-5 rounded-xl bg-[#f8fafc] p-4"><div className="flex items-center justify-between"><p className="text-sm font-black text-texto-principal">Auxílios</p><div className="flex items-center gap-2"><input value={novoBeneficio} onChange={(evento) => setNovoBeneficio(evento.target.value)} className="w-48 rounded-lg border border-slate-300 px-2 py-1.5 text-xs font-normal outline-none focus:border-azul-acao" placeholder="Novo auxílio global" /><button type="button" disabled={!novoBeneficio.trim()} onClick={() => { const nome = novoBeneficio.trim(); setNovoBeneficio(""); void onAdicionarBeneficio(nome); }} className="rounded-lg bg-azul-superficie px-2 py-1.5 text-xs font-black text-azul-principal disabled:opacity-50"><Plus className="inline size-3" /> Adicionar</button></div></div><div className="mt-3 grid grid-cols-2 gap-3">{catalogo.map((beneficio) => { const marcado = selecionado(beneficio.id); return <div key={beneficio.id} className={`rounded-xl border p-3 ${marcado ? "border-azul-secundario bg-white" : "border-slate-200 bg-white"}`}><div className="flex items-center justify-between gap-3"><label className="flex cursor-pointer items-center gap-2 text-sm font-bold text-texto-principal"><input type="checkbox" checked={Boolean(marcado)} onChange={() => alternarBeneficio(beneficio)} className="accent-azul-principal" />{beneficio.nome}</label><button type="button" onClick={() => onSolicitarExcluirBeneficio(beneficio)} className="text-slate-300 hover:text-red-500" title="Excluir de todas as escolas" aria-label="Excluir de todas as escolas"><Trash2 className="size-4" /></button></div>{marcado && <textarea rows={2} value={marcado.descricaoLocal} onChange={(evento) => onAtualizar({ ...turno, beneficios: turno.beneficios.map((item) => item.beneficioId === beneficio.id ? { ...item, descricaoLocal: evento.target.value } : item) })} className="mt-3 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs font-normal outline-none focus:border-azul-acao" placeholder="Texto específico para este turno" />}</div>; })}</div></div></article>;
}
...
function FormularioRefinado({ formulario, setFormulario, catalogo, onAdicionarBeneficio, onSolicitarExcluirBeneficio, onSalvar, onCancelar, processando }: { formulario: EscolaFormulario; setFormulario: React.Dispatch<React.SetStateAction<EscolaFormulario>>; catalogo: Beneficio[]; onAdicionarBeneficio: (nome: string, descricao: string) => Promise<boolean>; onSolicitarExcluirBeneficio: (beneficio: Beneficio) => void; onSalvar: () => Promise<void>; onCancelar: () => void; processando: boolean }) {
  ...
  const atualizar = <K extends keyof EscolaFormulario>(chave: K, valor: EscolaFormulario[K]) => {
    ...
    setErros((atual) => { const restante = { ...atual }; delete restante[chave]; return restante; });
  };
  ...
  const salvarBeneficio = async () => {
    const proximosErros: Record<string, string> = {};
    if (!nomeBeneficio.trim()) proximosErros.nome = "Informe o nome do auxílio.";
    if (!descricaoBeneficio.trim()) proximosErros.descricao = "Informe uma descrição para o auxílio.";
  ...
  const control = "mt-2 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco";
  ...
  return <div className="mt-7 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-5">
        <Campo titulo="Nome da escola" erro={erros.nome}>
          <div className="mt-2 flex min-w-0">
            <select value={formulario.sigla} onChange={(evento) => atualizar("sigla", evento.target.value)} className="h-11 w-28 shrink-0 rounded-l-xl border border-r-0 border-slate-300 bg-azul-superficie px-2 text-sm font-black text-azul-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco">
              <option value="">Sem sigla</option>{SIGLAS.map((sigla) => <option key={sigla} value={sigla}>{sigla}</option>)}
            </select>
            <input value={formulario.nome} onChange={(evento) => atualizar("nome", evento.target.value)} className="h-11 min-w-0 flex-1 rounded-r-xl border border-slate-300 bg-white px-3 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco" placeholder="Nome da escola" />
          </div>
        </Campo>
        ...
        <Campo titulo="Cidade"><select value={formulario.cidade} onChange={(evento) => { setFormulario((atual) => ({ ...atual, cidade: evento.target.value as EscolaFormulario["cidade"], bairro: "" })); setErros((atual) => { const restante = { ...atual }; delete restante.bairro; return restante; }); }} className={control}>{CIDADES.map((cidade) => <option key={cidade} value={cidade}>{cidade}</option>)}</select></Campo>
    ...
    <Card titulo="Níveis e Fotos" descricao="As fotos continuam hospedadas no GitHub: cole somente os links Raw.">
      <div className="grid grid-cols-[0.7fr_1.3fr] gap-8">
        <div><p className="text-sm font-bold text-texto-principal">Níveis oferecidos</p><div className="mt-3 space-y-3">{(["Ensino Fundamental", "Ensino Médio"] as Nivel[]).map((nivel) => <label key={nivel} className="flex min-h-11 items-center gap-3 rounded-xl border border-slate-200 px-3 text-sm font-bold text-texto-principal"><input type="checkbox" checked={formulario.niveis.includes(nivel)} onChange={() => atualizar("niveis", formulario.niveis.includes(nivel) ? formulario.niveis.filter((item) => item !== nivel) : [...formulario.niveis, nivel])} className="size-4 accent-azul-principal" />{nivel}</label>)}</div>{erros.niveis && <p className="mt-2 text-xs font-bold text-red-600">{erros.niveis}</p>}</div>
        <div><p className="text-sm font-bold text-texto-principal">Fotos (GitHub Raw)</p><div className="mt-3 space-y-3">{formulario.fotos.map((foto, indice) => <div key={`${indice}-${foto}`} className="flex h-11 items-center gap-2"><ImageIcon className="size-5 shrink-0 text-azul-secundario" /><input value={foto} onChange={(evento) => atualizar("fotos", formulario.fotos.map((item, posicao) => posicao === indice ? evento.target.value : item))} className="h-11 min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco" placeholder={`Link ${indice + 1}`} />{formulario.fotos.length > 1 && <button type="button" onClick={() => atualizar("fotos", formulario.fotos.filter((_, posicao) => posicao !== indice))} className="rounded-lg p-2 text-red-500 hover:bg-red-50" aria-label="Remover foto"><Trash2 className="size-5" /></button>}</div>)}</div><Botao variante="borda" onClick={() => atualizar("fotos", [...formulario.fotos, ""])} className="mt-4"><Plus className="size-4" />Adicionar outra foto</Botao></div>
      </div>
    </Card>
    ...
    {modalBeneficio && <Modal titulo="Adicionar Auxílio Padrão" onFechar={() => setModalBeneficio(false)}><p className="text-sm leading-6 text-slate-500">Este auxílio padrão ficará disponível para as próximas escolas e turnos.</p><div className="mt-4 space-y-4"><Campo titulo="Nome do Auxílio" erro={errosBeneficio.nome}><input value={nomeBeneficio} onChange={(evento) => setNomeBeneficio(evento.target.value)} className={control} placeholder="Ex.: Transporte escolar" /></Campo><Campo titulo="Descrição" erro={errosBeneficio.descricao}><textarea rows={4} value={descricaoBeneficio} onChange={(evento) => setDescricaoBeneficio(evento.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco" placeholder="Explique o auxílio oferecido" /></Campo></div><div className="mt-5 flex justify-end gap-3"><Botao variante="borda" onClick={() => setModalBeneficio(false)}>Cancelar</Botao><Botao disabled={processando} onClick={() => void salvarBeneficio()}><Check className="size-4" />Salvar</Botao></div></Modal>}
  </div>;
}
...
function CartaoTurnoRefinado({ turno, indice, niveis, anterior, catalogo, erros, onAtualizar, onRemover, onSolicitarExcluirBeneficio, onAbrirModalBeneficio }: { turno: TurnoFormulario; indice: number; niveis: Nivel[]; anterior?: TurnoFormulario; catalogo: Beneficio[]; erros: { horario?: string; dias?: string; descricao?: string }; onAtualizar: (turno: TurnoFormulario) => void; onRemover: () => void; onSolicitarExcluirBeneficio: (beneficio: Beneficio) => void; onAbrirModalBeneficio: () => void }) {
  ...
  const alternarBeneficio = (beneficio: Beneficio) => onAtualizar({ ...turno, beneficios: selecionado(beneficio.id) ? turno.beneficios.filter((item) => item.beneficioId !== beneficio.id) : [...turno.beneficios, { beneficioId: beneficio.id, descricaoLocal: beneficio.descricao }] });
  const control = "mt-2 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco";
  return <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_24px_-16px_rgba(2,87,160,0.25)]"><div className="flex items-center justify-between gap-4"><h3 className="font-black text-azul-principal">Turno {indice + 1}</h3><div className="flex items-center gap-3">{anterior && <button type="button" onClick={() => onAtualizar({ ...turno, comoFunciona: anterior.comoFunciona, beneficios: anterior.beneficios.map((beneficio) => ({ ...beneficio })) })} className="inline-flex min-h-10 items-center gap-1 rounded-lg bg-violet-50 px-3 py-2 text-xs font-black text-violet-700 hover:bg-violet-100"><Copy className="size-3.5" />Copiar informações do turno anterior</button>}<button type="button" onClick={onRemover} className="rounded-lg p-2 text-red-500 hover:bg-red-50" aria-label="Remover turno"><Trash2 className="size-5" /></button></div></div><div className="mt-5 grid grid-cols-3 gap-4"><Campo titulo="Nível deste turno"><select value={turno.nivel} onChange={(evento) => onAtualizar({ ...turno, nivel: evento.target.value as Nivel })} className={control}>{niveis.map((nivel) => <option key={nivel} value={nivel}>{nivel}</option>)}</select></Campo><Campo titulo="Turno"><select value={turno.turno} onChange={(evento) => onAtualizar({ ...turno, turno: evento.target.value as TurnoFormulario["turno"] })} className={control}><option>Manhã</option><option>Tarde</option><option>Noite</option></select></Campo><Campo titulo="Horário" dica={`Grava como: ${turno.inicio || "HH:MM"} - ${turno.fim || "HH:MM"}`} erro={erros.horario}><div className="mt-2 flex h-11 items-center gap-2"><Clock3 className="size-4 shrink-0 text-azul-secundario" /><input type="time" value={turno.inicio} onChange={(evento) => onAtualizar({ ...turno, inicio: evento.target.value })} className="h-11 w-full rounded-xl border border-slate-300 px-2 font-normal outline-none focus:border-azul-acao focus:ring-4 focus:ring-azul-foco" /><span>–</span><input type="time" value={turno.fim} onChange={(evento) => onAtualizar({ ...turno, fim: evento.target.value })} className="h-11 w-full rounded-xl border border-slate-300 px-2 font-normal outline-none focus:border-azul-acao focus:ring-4 focus:ring-azul-foco" /></div></Campo></div><div className="mt-5 grid grid-cols-2 gap-4"><Campo titulo="Dias de aula" dica={`Grava como: ${abreviarDias(turno.diaInicio, turno.diaFim) || "Seg - Sex"}`} erro={erros.dias}><div className="mt-2 flex h-11 items-center gap-2"><span className="text-sm font-normal text-slate-500">De</span><select value={turno.diaInicio} onChange={(evento) => onAtualizar({ ...turno, diaInicio: evento.target.value as Dia })} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal outline-none focus:border-azul-acao focus:ring-4 focus:ring-azul-foco"><option value="">–</option>{DIAS.map((dia) => <option key={dia} value={dia}>{dia}</option>)}</select><span className="text-sm font-normal text-slate-500">a</span><select value={turno.diaFim} onChange={(evento) => onAtualizar({ ...turno, diaFim: evento.target.value as Dia })} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal outline-none focus:border-azul-acao focus:ring-4 focus:ring-azul-foco"><option value="">–</option>{DIAS.map((dia) => <option key={dia} value={dia}>{dia}</option>)}</select></div></Campo><Campo titulo="Como funciona" erro={erros.descricao}><textarea rows={4} value={turno.comoFunciona} onChange={(evento) => onAtualizar({ ...turno, comoFunciona: evento.target.value })} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 font-normal text-texto-principal outline-none transition focus:border-azul-acao focus:ring-4 focus:ring-azul-foco" placeholder="Descreva como funciona este turno" /></Campo></div><div className="mt-5 rounded-xl bg-[#f8fafc] p-4"><div className="flex items-center justify-between gap-4"><p className="text-sm font-black text-texto-principal">Auxílios</p><Botao onClick={onAbrirModalBeneficio} className="!px-3 !py-2"><Plus className="size-4" />Adicionar Auxílio Padrão</Botao></div><div className="mt-3 grid grid-cols-2 gap-3">{catalogo.map((beneficio) => { const marcado = selecionado(beneficio.id); return <div key={beneficio.id} className={`rounded-xl border p-3 ${marcado ? "border-azul-secundario bg-white" : "border-slate-200 bg-white"}`}><div className="flex items-center justify-between gap-3"><label className="flex cursor-pointer items-center gap-2 text-sm font-bold text-texto-principal"><input type="checkbox" checked={Boolean(marcado)} onChange={() => alternarBeneficio(beneficio)} className="accent-azul-principal" />{beneficio.nome}</label><button type="button" onClick={() => onSolicitarExcluirBeneficio(beneficio)} className="text-slate-300 hover:text-red-500" title="Excluir de todas as escolas" aria-label="Excluir de todas as escolas"><Trash2 className="size-4" /></button></div>{marcado && <textarea rows={2} value={marcado.descricaoLocal} onChange={(evento) => onAtualizar({ ...turno, beneficios: turno.beneficios.map((item) => item.beneficioId === beneficio.id ? { ...item, descricaoLocal: evento.target.value } : item) })} className="mt-3 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs font-normal outline-none focus:border-azul-acao" placeholder="Texto específico para este turno" />}</div>; })}</div></div></article>;
}
