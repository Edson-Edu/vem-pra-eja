...
const BAIRROS: Record<EscolaFormulario["cidade"], string[]> = {
  Camboriú: ["Centro", "Areias", "Barra", "Cedro", "Lídia Duarte", "Monte Alegre", "Rio Pequeno", "Santa Regina", "São Francisco de Assis", "Taboleiro"],
  "Balneário Camboriú": ["Ariribá", "Barra", "Centro", "Estados", "Municípios", "Nações", "Nova Esperança", "Pioneiros", "Praia dos Amores", "Vila Real"],
};
...
export default function PainelAdministrativo() {
  ...
  const removerBeneficioGlobal = async () => {
    ...
    try {
      ...
      mostrarMensagem("Não foi possível remover o benefício global.", "erro");
    } finally { setProcessando(false); }
  };
  const adicionarBeneficio = async (nome: string, descricao: string) => {
    ...
    try {
      ...
      mostrarMensagem("Não foi possível criar o benefício global.", "erro");
      return false;
    } finally { setProcessando(false); }
  };
  ...
  return <main className="min-h-dvh bg-[#f2f3f6] text-[#1e293b]">
    ...
    {modalExcluirBeneficio && <Modal titulo="Excluir benefício de todas as escolas" perigo onFechar={() => setModalExcluirBeneficio(null)}><p className="text-sm leading-6 text-slate-600">Ao confirmar, <strong>{modalExcluirBeneficio.nome}</strong> será removido de todas as escolas e turnos. Essa ação não poderá ser desfeita.</p><div className="mt-5 flex justify-end gap-3"><Botao variante="borda" onClick={() => setModalExcluirBeneficio(null)}>Cancelar</Botao><Botao variante="perigo" disabled={processando} onClick={() => void removerBeneficioGlobal()}>Excluir de todas</Botao></div></Modal>}
