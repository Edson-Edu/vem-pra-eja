...
function NumeroAnimado({ valor }: { valor: number }) {
  ...
  return <motion.span initial={{ opacity: 0, scale: 0.75 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: "spring", stiffness: 180, damping: 16 }} className="inline-block text-azul-acao [text-shadow:0_0_28px_rgba(0,139,255,0.44)]">{numero.toLocaleString("pt-BR")}</motion.span>;
}
...
function ElementoVisual({ tipo, Icon }: { tipo: string; Icon: typeof Sparkles }) {
  const gradientes: Record<string, string> = {
    inscricao: "from-azul-acao via-azul-secundario to-azul-principal",
    noite: "from-azul-principal via-azul-secundario to-azul-acao",
    bairro: "from-azul-acao via-azul-principal to-azul-secundario",
  };
  return <motion.div animate={{ y: [0, -16, 0], rotate: [0, 2, 0] }} transition={{ duration: 5.5, repeat: Infinity, ease: "easeInOut" }} className="relative mx-auto grid aspect-square w-full max-w-[480px] place-items-center">
    ...
    <i className="absolute inset-[19%] rounded-full border border-azul-superficie/60" />
    <i className="absolute inset-[31%] rounded-full border border-azul-foco/70" />
    <motion.i animate={{ rotate: 360 }} transition={{ duration: 24, repeat: Infinity, ease: "linear" }} className="absolute inset-0 rounded-full border border-dashed border-azul-superficie/50" />
...
function FotoDaEscola({ foto, nome }: { foto: string; nome: string }) {
  ...
  return <motion.figure initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.18, duration: 0.7, ease: [0.16, 1, 0.3, 1] }} className="relative mx-auto w-full max-w-[590px]">
    <motion.div animate={{ y: [0, -12, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="overflow-hidden rounded-[2.4rem] border border-white/35 bg-azul-superficie p-2 shadow-[0_30px_80px_-28px_rgba(0,0,0,0.7)]">
      {/* A URL é cadastrada dinamicamente e a exportação estática não possui um
          carregador de imagens; manter img preserva a retrospectiva existente. */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={foto} alt={`Foto da ${nome}`} className="aspect-[16/10] w-full rounded-[1.9rem] object-cover" />
    </motion.div>
    <figcaption className="absolute -bottom-5 left-8 rounded-xl border border-white/35 bg-azul-principal px-5 py-3 text-sm font-black text-white shadow-xl">Escola com mais inscrições</figcaption>
  </motion.figure>;
}
...
function DispositivosVisuais({ dispositivo }: { dispositivo: string }) {
  ...
  return <div className="relative mx-auto flex h-[390px] w-full max-w-[560px] items-center justify-center">
    <motion.div initial={{ opacity: 0, scale: 0.78, filter: "blur(12px)" }} animate={{ opacity: [0, 1, 0], scale: [0.78, 1, 0.92], filter: ["blur(12px)", "blur(0px)", "blur(6px)"] }} transition={{ duration: 1.55, times: [0, 0.35, 1], ease: [0.16, 1, 0.3, 1] }} className="absolute grid size-40 place-items-center rounded-full border border-white/35 bg-white/10 text-7xl font-black text-azul-superficie">?</motion.div>
    <motion.div initial={{ opacity: 0, y: 52, scale: 0.68 }} animate={{ opacity: 1, y: [18, -10, 0], scale: [0.68, 1.08, 1] }} transition={{ delay: 1.15, duration: 1.05, ease: [0.16, 1, 0.3, 1] }} className="relative grid size-[300px] place-items-center rounded-[4rem] border border-white/45 bg-gradient-to-br from-azul-acao to-azul-secundario shadow-[0_32px_90px_rgba(0,0,0,0.38)]">
      <motion.i animate={{ scale: [1, 1.08, 1], opacity: [0.35, 0.7, 0.35] }} transition={{ duration: 3.8, repeat: Infinity, ease: "easeInOut" }} className="absolute -inset-7 rounded-[5rem] border border-azul-superficie/60" />
      <IconePrincipal className={["relative text-white", computador ? "size-40" : tablet ? "size-36" : "size-32"].join(" ")} strokeWidth={1.15} />
    </motion.div>
    <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.05, duration: 0.5 }} className="absolute -bottom-2 rounded-xl border border-white/30 bg-white/15 px-5 py-3 text-lg font-black text-azul-superficie">Aparelho mais usado: <span className="text-white">{nome}</span></motion.p>
  </div>;
}
...
export default function RetrospectivaAcessos({ dados, periodo, inicio, fim, textoDoPeriodo, aoFechar }: RetrospectivaProps) {
  ...
  const frase = () => {
    if (slide.id === "abertura") return <div className="text-center"><motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 150, damping: 15 }} className="mx-auto mb-8 grid size-24 place-items-center rounded-[2rem] border border-white/35 bg-white/15 shadow-[0_0_70px_rgba(0,139,255,0.55)]"><Sparkles className="size-11 text-azul-superficie" /></motion.div><h2 className="text-6xl font-black tracking-[-0.055em] text-white 2xl:text-8xl">Vem pra EJA<br /><span className="text-azul-acao">em números.</span></h2><motion.p initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.42 }} className="mx-auto mt-7 max-w-2xl text-xl leading-8 text-azul-superficie">Uma retrospectiva sobre as pessoas que buscaram novas oportunidades pelo estudo.</motion.p></div>;
    ...
    if (slide.id === "escola") return <p className={tituloGrande}><TextoRevelado texto="A escola com mais inscrições no período foi" /> <span className="inline-block text-azul-acao [text-shadow:0_0_28px_rgba(0,139,255,0.44)]">{dadosDaHistoria.escola}</span><span>.</span></p>;
    if (slide.id === "favorita") return <div><p className="mb-7 text-xl font-bold text-azul-foco"><TextoRevelado texto="Entre as pessoas que concluíram a inscrição," /></p><p className={tituloGrande}><span className="text-azul-acao">{dadosDaHistoria.nivel}</span> <TextoRevelado texto="no turno da" /> <span className="text-azul-acao">{dadosDaHistoria.turno}</span> <TextoRevelado texto="foi a combinação mais escolhida." /></p></div>;
    if (slide.id === "bairro") return <p className={tituloGrande}><TextoRevelado texto="A região que mais teve alunos se inscrevendo foi o bairro" /> <span className="inline-block text-azul-acao">{dadosDaHistoria.bairro}</span><span>.</span></p>;
    if (slide.id === "dispositivo") return <div className="text-center"><p className={tituloGrande}><TextoRevelado texto="O horário mais comum para iniciar a inscrição foi por volta das" /> <span className="text-azul-acao">{dadosDaHistoria.horario}</span>.</p><p className="mx-auto mt-7 max-w-4xl text-2xl font-bold leading-9 text-azul-superficie"><TextoRevelado texto="E o acesso aconteceu principalmente pelo" /> <span className="text-white">{dadosDaHistoria.dispositivo}</span>.</p></div>;
    return <div className="text-center"><motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 150, damping: 15 }} className="mx-auto mb-8 grid size-24 place-items-center rounded-[2rem] border border-white/35 bg-white/15"><GraduationCap className="size-12 text-azul-superficie" /></motion.div><h2 className="max-w-5xl text-balance text-5xl font-black leading-[1.03] tracking-[-0.055em] text-white xl:text-7xl">Cada número representa uma <span className="text-azul-acao">nova oportunidade.</span></h2></div>;
  };
  ...
  return <AnimatePresence><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] overflow-hidden bg-azul-principal text-white">
    ...
    <div className="absolute left-10 top-14 z-20 flex items-center gap-3 text-sm font-black tracking-wide text-azul-superficie"><span className="grid size-9 place-items-center rounded-xl bg-azul-acao shadow-[0_0_24px_rgba(0,139,255,0.7)]"><Play className="size-4 fill-white" /></span>RETROSPECTIVA <span className="font-medium text-white/70">• {textoDoPeriodo}</span></div>
    ...
    <div className="absolute bottom-9 left-10 z-20 flex items-center gap-3 text-xs font-bold text-white/75"><GraduationCap className="size-4" />Vem pra EJA <span className="size-1 rounded-full bg-azul-superficie" /> dados do período selecionado</div>
