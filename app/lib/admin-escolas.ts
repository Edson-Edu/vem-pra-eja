...
export async function criarBeneficioAdministrativo(nome: string, descricao: string) { const dados = await requisicao("beneficios_catalogo", { method: "POST", headers: { "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify({ nome, descricao_padrao: descricao }) }); const linha = lista(dados)[0] as Linha | undefined; if (!linha) throw new Error("Não foi possível criar o benefício."); return { id: valorTexto(linha.id), nome: valorTexto(linha.nome), descricao: valorTexto(linha.descricao_padrao) }; }
