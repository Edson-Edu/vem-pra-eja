...
class TelaCadastro extends StatefulWidget {
  final String idEscolaSelecionada; // NOVO: Recebe o ID
...
class _TelaCadastroState extends State<TelaCadastro> {
  ...
  bool _enviandoDados = false; // NOVO: Controle do botão de salvar
  ...
  Future<Iterable<Map<String, String>>> _sugerirRuasGlobal(String termo) async {
    if (termo.length < 3 || _camposEndTravados) {
  ...
  // NOVO: Função Mestre que salva a inscrição no banco
  Future<void> _enviarInscricao() async {
    ...
    try {
      await Supabase.instance.client.from('inscricoes').insert({
        ...
        'cpf': _cpfCtrl.text.replaceAll(RegExp(r'\D'), ''), // Salva só os números
        'ddd': _dddCtrl.text,
      ...
      pararVoz();
      if (mounted) {
        // Redireciona para sucesso sem deixar voltar para o form
      ...
      // Captura o erro específico de CPF duplicado gerado pela trava do banco
      if (e.toString().contains('inscricoes_cpf_key') || e.toString().contains('unique constraint')) {
      ...
      appBar: AppBar(
        ...
        actions: const [
          BotaoAcessibilidadeGlobal(
            textoLeituraTela: "Preencha seus dados de contato. Ao finalizar, clique em Continuar para preencher o endereço.",
          )
        ],
      ),
      body: SafeArea(
        child: Padding(
          ...
          child: ListView(
            ...
            children: [
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    ...
                                    child: TextFormField(
                                      ...
                                      onChanged: (v) { 
                                        if (v.length == 2) {
                                          FocusScope.of(context).requestFocus(_focoNumero);
                                        }
                                      },
                                  ...
                                  Expanded(
                                    child: TextFormField(
                                      ...
                                      keyboardType: TextInputType.number,
                                      inputFormatters: [MascaraWhatsApp()],
                                      decoration: InputDecoration(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                items: _cidadesPermitidas.map((c) {
                              ...
                              RawAutocomplete<Map<String, String>>(
                                optionsBuilder: (TextEditingValue textEditingValue) {
                                  return _sugerirRuasGlobal(textEditingValue.text);
                                },
                                displayStringForOption: (option) => option['rua']!,
                                onSelected: (Map<String, String> selection) {
                                  setState(() {
                                    _ruaCtrl.text = selection['rua']!;
                                    _bairroCtrl.text = selection['bairro']!;
                                    _cepCtrl.text = selection['cep']!;
                                    _cidadeSelecionada = selection['cidade']!;
                                    _camposEndTravados = true;
                                    FocusScope.of(context).requestFocus(_focoNumeroEndereco);
                                  });
                                },
                                fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
                                  if (controller.text.isEmpty && _ruaCtrl.text.isNotEmpty) {
              ...
              // NOVO: Botão com controle de estado de salvamento
              FadeInUp(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: _enviandoDados ? null : () {
                      ...
                      if (contatoValido && enderecoValido) {
                        _enviarInscricao(); // Chama a função de inserção no banco
                      } else {
                        setState(() {
                          if (!contatoValido) {
                            _contatoAberto = true;
...
class MascaraWhatsApp extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    String t = p;
    if (p.length > 5) {
      t = '${p.substring(0, 5)}-${p.substring(5)}';
    }
